...
export default function Header() {
  ...
  return (
    <header
      className={`fixed left-1/2 -translate-x-1/2 z-50 transition-all duration-500 w-[95%] md:w-[90%] max-w-5xl ${scrolled
    ...
    >
      <nav
        className={`px-4 md:px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? "h-16" : "h-20"
          }`}
      >
        <Link href="/" className="flex items-center gap-2 cursor-pointer group" data-cursor="hover">
          <div
            className={`rounded-full flex items-center justify-center font-bold text-xs transition-all duration-300 bg-black text-white border border-white/20 group-hover:bg-white group-hover:text-black group-hover:scale-110 ${scrolled ? "w-8 h-8" : "w-10 h-10"
              }`}
        ...
        <ul className="hidden md:flex items-center gap-8 text-xs font-bold uppercase tracking-widest text-gray-400">
          {navLinks.map((link) => (
            ...
            >
              <Link
                href={link.href}
                className={`relative z-10 px-3 py-2 transition-colors duration-300 ${hoveredLink === link.name ? "text-white" : ""
                  }`}
                data-cursor="hover"
              >
                {link.name}
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <div className="relative hidden sm:block">
            <Link href="/#contact" data-cursor="hover">
              <div
                className={`relative overflow-hidden bg-brand text-white px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest group-hover:text-white transition-all duration-500 hover:scale-105 hover:shadow-[0_0_20px_rgba(139,92,246,0.6)] z-10 ${scrolled ? "shadow-[0_0_15px_rgba(139, 92, 246,0.3)]" : ""
                  }`}
              >
